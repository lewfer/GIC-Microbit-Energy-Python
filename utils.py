...
def text(screen, x,y,t,size=24):
    font = pygame.font.Font('freesansbold.ttf',size)
    text = font.render(t, True, black)
    text_rect = text.get_rect(center=(x, y))
    screen.blit(text, text_rect)

def textLeft(screen, x,y,t,size=24):
    font = pygame.font.Font('freesansbold.ttf',size)
    text = font.render(t, True, black)
...
class Graph():
    ...
    def plot(self, data,color):
        ...
        if len(data)>0:
            ...
            for i,d in enumerate(data):
                x = map(i,self.x1,self.x2, 0, self.rect.w) + self.rect.x
                y = map(d, self.y1,self.y2, self.rect.h,0) + self.rect.y
    ...
    def xAxis(self,x1,x2):
        ...
        for i in range(x1,x2,step):
            x = map(i,x1,x2, 0, self.rect.w) + self.rect.x
            text(self.screen, x,self.rect.y+self.rect.w,str(i), size=8)

    def yAxis(self,y1,y2):
        step = int((y2-y1)/10)   #max(int(count/10),1)
        for i in range(y1,y2,step):
            y = map(i,y1,y2, self.rect.h,0) + self.rect.y
            text(self.screen, self.rect.x,y,str(i), size=10)            
